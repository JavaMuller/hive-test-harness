...
# Vagrantfile generated by https://github.com/timveil/hdp-vagrant-veil.hdp.generator on 6/6/16 1:49 PM
...
Vagrant.configure("2") do |config|
    ...
    if FFI::Platform::IS_MAC
        ...
        config.multihostsupdater.aliases = {'192.168.7.101' => ['veil.hdp.hive-test.hdp.local']}
    ...
    config.vm.hostname = 'veil.hdp.hive-test.hdp.local'
    ...
    config.vm.provider "virtualbox" do |v|
        v.memory = 8192
        v.cpus = 4
        v.name = 'veil.hdp.hive-test.hdp.local'
    end
end
...
192.168.7.101   veil.hdp.hive-test.hdp.local
EOF
...
sed -i "s/^hostname=localhost/hostname=veil.hdp.hive-test.hdp.local/g" /etc/ambari-agent/conf/ambari-agent.ini
...
curl -s --show-error -H "X-Requested-By: ambari" -X POST -d '{"ViewInstanceInfo":{"instance_name":"FILES_VIEW_INSTANCE","label":"Files View","visible":true,"icon_path":"","icon64_path":"","properties":{"webhdfs.url":"webhdfs://veil.hdp.hive-test.hdp.local:50070","webhdfs.username":null,"webhdfs.auth":"auth=SIMPLE"},"description":"Files View installed by https://github.com/timveil/hdp-vagrant-veil.hdp.generator"}}' -u admin:admin http://veil.hdp.hive-test.hdp.local:8080/api/v1/views/FILES/versions/1.0.0/instances/Files
...
curl -s --show-error -H "X-Requested-By: ambari" -X POST -d '{"blueprint" : "test","default_password" : "password","host_groups" :[{"name" : "host_group_1","hosts" : [{"fqdn" : "veil.hdp.hive-test.hdp.local"}]}]}' -u admin:admin http://veil.hdp.hive-test.hdp.local:8080/api/v1/clusters/veil.hdp.hive-test
...
usermod -a -G veil.hdp.hive admin
...
usermod -a -G veil.hdp.hive vagrant
...
echo " "
echo "---------------------------------------------------------------------------------------------------------------"
echo "----- Cluster Install Complete!  http://veil.hdp.hive-test.hdp.local:8080"
echo "---------------------------------------------------------------------------------------------------------------"
echo " "
