CREATE TABLE ft_je_amounts (
  je_id string,
  coa_id INT,
  period_id INT,
  bu_id INT,
  source_id INT,
  user_listing_id INT,
  approved_by_id INT,
  segment1_id INT,
  segment2_id INT,
  dr_cr_ind string,
  reversal_ind string,
  sys_manual_ind string,
  Journal_type string,
  year_flag string,
  period_flag string,
  ey_period string,
  entry_date_id INT,
  effective_date_id INT,
  Net_amount FLOAT,
  Net_amount_debit FLOAT,
  Net_amount_credit FLOAT,
  amount_curr_cd string,
  Net_reporting_amount FLOAT,
  Net_reporting_amount_debit FLOAT,
  Net_reporting_amount_credit FLOAT,
  reporting_amount_curr_cd string,
  Net_functional_amount FLOAT,
  Net_functional_debit_amount FLOAT,
  Net_functional_credit_amount FLOAT,
  functional_amount_curr_cd string,
  count_je_id INT
) stored AS orc;

CREATE TABLE ft_je_amounts_csv (
  je_id string,
  coa_id INT,
  period_id INT,
  bu_id INT,
  source_id INT,
  user_listing_id INT,
  approved_by_id INT,
  segment1_id INT,
  segment2_id INT,
  dr_cr_ind string,
  reversal_ind string,
  sys_manual_ind string,
  Journal_type string,
  year_flag string,
  period_flag string,
  ey_period string,
  entry_date_id INT,
  effective_date_id INT,
  Net_amount FLOAT,
  Net_amount_debit FLOAT,
  Net_amount_credit FLOAT,
  amount_curr_cd string,
  Net_reporting_amount FLOAT,
  Net_reporting_amount_debit FLOAT,
  Net_reporting_amount_credit FLOAT,
  reporting_amount_curr_cd string,
  Net_functional_amount FLOAT,
  Net_functional_debit_amount FLOAT,
  Net_functional_credit_amount FLOAT,
  functional_amount_curr_cd string,
  count_je_id INT
) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\054' stored AS textfile;

load data inpath '/poc/data/ey/ft_je_amounts.csv' into table ft_je_amounts_csv;

insert into table ft_je_amounts  select * from ft_je_amounts_csv;
