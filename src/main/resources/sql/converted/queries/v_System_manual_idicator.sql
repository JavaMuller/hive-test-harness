...
FROM			 [Parameters_Source_listing] P_SL
INNER JOIN		 [Source_listing] SL				ON		P_SL.source_cd = SL.source_cd
UNION ALL
SELECT
...
FROM			 [Parameters_User_listing] P_UL
INNER JOIN		 [User_listing] UL				ON		P_UL.client_user_id = UL.client_user_id
...
FROM				 SOURCE_USER_CTE SRC_USR
  INNER JOIN			 [Parameters_engagement] PE		ON		SRC_USR.table_identifier = PE.system_manual_indicator_option
